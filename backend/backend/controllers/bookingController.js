...
const getAvailableTimeSlots = async (req, res) => {
  try {
    ...
    // Generate all possible time slots from 10:00 to 19:00 (program de lucru)
    const timeSlots = [];
    const startHour = 10;
    const endHour = 19;
    ...
    if (timeSlots.length === 0) {
      return res.status(200).json({ 
        success: true, 
        timeSlots: [],
        message: 'Nu există intervale orare disponibile pentru data selectată. Vă rugăm să alegeți o altă dată.'
      });
    }
...
const createBooking = async (req, res) => {
  try {
    ...
    if (!isAvailable) {
      return errorResponse(res, 400, 'Intervalul orar selectat nu mai este disponibil. Vă rugăm să alegeți alt interval.');
    }
